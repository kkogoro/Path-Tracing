    ...
    Scene LoadScene(std::filesystem::path const & fileName) {
        ...
        if (root["Lights"]) {
            for (auto const & lightNode : root["Lights"]) {
                ...
                SetValue(light.Length, lightNode["Length"]);
                SetValue(light.p[0], lightNode["p0"]);
                SetValue(light.p[1], lightNode["p1"]);
                SetValue(light.p[2], lightNode["p2"]);
        ...
        if (root["Materials"]) {
            for (auto const & materialNode : root["Materials"]) {
                ...
                SetValue(material.Emission, materialNode["Emission"]);
